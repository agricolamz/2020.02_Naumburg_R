...
The `str_c()` function is vectorised, so it returns the number of strings that it got to the input. It is possible to merge all merged strings into one biger string using the `collapse` argument.
...
Extract substring from a string: по индексам функцией `str_sub()`, и по подстроке функцией `str_extract()`.
