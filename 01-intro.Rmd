...
```{block, type = "rmdtask"}
How many elements in the vector `g` if expression `g[pi < 1000]` do not return an error?
```
...
Not turning on an R package is the most popular mistake of my students. So remmember:
...
A data frame is a collection of variables of the same number of rows with unique row names.

```{r}
tibble(name = c("Małgorzata"))
```
...
```{block, type = "rmdtask"}
Because of 2019–20 Wuhan coronavirus outbreak the city of Wuhan is everywhere on media. [In Russian](https://nplus1.ru/blog/2020/02/03/wuhan) for some reason Wuhan is masculine sometimes and sometimes it's feminin. I looked into other Slavic languages and recorded obtained data into the [`.csv` file](https://gist.githubusercontent.com/agricolamz/c280527d6b5d79693b85d8fcf8d35bc3/raw/c46a32a218bc5d6933703ae677b81ff5a4a1dcaa/wuhan.csv). Download this files to R. What variables does it have?
```
