...
```{block, type = "rmdtask"}
In built-in dataset `starwars` filters those characters that are higher then 180 (`height`) and weigh less then 80 (`mass`). Then get a unique name of their homeworlds (`homeworld`).
```
...
```{block2, type = "rmdtask"}
Creates a variable with body mass index [индексом Кетле](https://en.wikipedia.org/wiki/Body_mass_index):  $\frac{mass}{height^2}$ for all characters from `starwars` dataset. How many charachters have obesity (have body mass index greater 30)? (Don't forget to convert height from centimetres to metres).
```
